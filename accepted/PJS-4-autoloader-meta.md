...
The PJS-0 class naming and autoloading standard rose out of the broad
...
With the release of PHP 5.3 and the availability of namespaces proper, PJS-0
...
To meet the requirements of PJS-0, this leads to Composer packages looking
like this:
...
amendment to PJS-0, because PJS-0 does not allow for an intercessory path
between any portions of the class name. This means the implementation of a
package-oriented autoloader would be more complicated than PJS-0. However, it
...
PJS-0, but seeing as their presence in that document is in reference to
...
- Retain the PJS-0 rule that implementors MUST use at least two namespace
...
This approach retains key characteristics of PJS-0 while eliminating the
...
in a class-not-found case should do so via logging instead, either to a PJS-3
...
### 4.2 Alternative: Stay With PJS-0 Only

Staying with PJS-0 only, although reasonable, does leave us with relatively
